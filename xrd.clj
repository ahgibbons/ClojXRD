...
(defn metrix-tensor [lattice-params]
  (let [a (lattice-params :a)
        b (lattice-params :b)
        c (lattice-params :c)
        alpha (deg->rad (lattice-params :alpha))
        beta (deg->rad (lattice-params :beta))
        gamma (deg->rad (lattice-params :gamma))]
...
(defn metric-term [gik ri qi rk qk]
  (* gik (- ri qi) (- rk qk)))

(defn distance-cryst [mtensor q r]
  (let [term (fn [i k] (*  (M_ij mtensor i k) (- (nth r i) (nth q i)) (- (nth r k) (nth q k))))]
    (Math/sqrt (reduce + (map (fn [ik] (term (nth ik 0) (nth ik 1))) (cartesian-prod '((0 1 2) (0 1 2))))))))

(defn angle-cryst [mtensor q p r]
  (let [term (fn [i k]
               (* (M_ij mtensor i k)
                  (- (nth q i) (nth p i))
                  (- (nth r k) (nth p k))))
        r-pq-inv (/ 1 (distance-cryst mtensor p q))
        r-pr-inv (/ 1 (distance-cryst mtensor p r))]
    (* r-pq-inv
       r-pr-inv
       (reduce + (map (fn [ik] (term (nth ik 0) (nth ik 1))) (cartesian-prod '((0 1 2) (0 1 2))))))))

;;;; Testing

(def simple-cube-metric [[9 0 0] [0 9 0] [0 0 9]])
